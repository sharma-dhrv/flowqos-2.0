...
class FlowQoS (EventMixin):
  def __init__ (self):
    ...
    self.west_service_to_port = { 'VIDEO': 4,
                        'VOIP': 5,
                        'Game': 6,
                        'WEB': 7,
                      }
    #self.east_dpid = int("0x0000baf8d9adad9f",16)
    self.east_dpid = int("0x000066b16f7a7495",16)
    self.east_home_port = 1
    #self.west_dpid = int("0x000004a151a30fcc",16)
    self.west_dpid = int("0x0000baf8d9adad9f",16)
    ...
    if packet.type != ethernet.ARP_TYPE and packet.type != ethernet.IPV6_TYPE:
      if packet.next.protocol != ipv4.ICMP_PROTOCOL:
        if packet.next.next.dstport == 80 or packet.next.next.dstport == 443:
          ...
          f=open ('/home/dhsharma/project/flow_dict2.json','r') 
          for obj in json_decoder(f): 
            ...
            thefile = open("/home/dhsharma/project/log_flows.txt", "w")
            for item in flows:
              thefile.write("%s\n" % item)
            thefile.close()
            ...
            thefile2 = open("/home/dhsharma/project/log_classification.txt", "w")
                ...
                if type(key) is not str:
                    try:
                        ...
                        try:
                          ...
                          pass
            f2=open ('/home/dhsharma/project/flow_dict2.json','w+')
